...
@patch("code_challenges.advent_of_code.security_through_obscurity.get_room_names")
def test_get_incrypted_room_name(mock_room_name_input):
    mock_room_name_input.return_value = [
        "aaaaa-bbb-z-y-x-123[abxyz]",
        "a-b-c-d-e-f-g-h-987[abcde]",
        "not-a-real-room-404[oarel]",
    ...
    actual = get_incrypted_room_name()
    expected = (
        1514,
        [
            ["aaaaa", "bbb", "z", "y", "x", "123[abxyz]"],
...
@patch("code_challenges.advent_of_code.security_through_obscurity")
def test_get_room_names(mock_room_name_input):
    mock_room_name_input.return_value = "aczupnetwp-mfyyj-opalcexpye-977[peyac]\nqzchnzbshud-cxd-trdq-sdrshmf-105[jqexn]\nmolgbzqfib-bdd-mrozexpfkd-289[bdfmo]"
